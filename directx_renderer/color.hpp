...
struct color_t
{
	...
	inline uint8_t* a() { return reinterpret_cast<uint8_t*>(&color + 3); } //-V206
	inline uint8_t* r() { return reinterpret_cast<uint8_t*>(&color + 2); } //-V206
	inline uint8_t* g() { return reinterpret_cast<uint8_t*>(&color + 1); } //-V206
	inline uint8_t* b() { return reinterpret_cast<uint8_t*>(&color + 0); } //-V206
	...
	inline void set_a(const float a) { reinterpret_cast<uint8_t*>(&color)[3] = static_cast<uint8_t>(a * 255.f); } //-V206
	inline void set_r(const float r) { reinterpret_cast<uint8_t*>(&color)[2] = static_cast<uint8_t>(r * 255.f); } //-V206
	inline void set_g(const float g) { reinterpret_cast<uint8_t*>(&color)[1] = static_cast<uint8_t>(g * 255.f); } //-V206
	inline void set_b(const float b) { reinterpret_cast<uint8_t*>(&color)[0] = static_cast<uint8_t>(b * 255.f); } //-V206

	color_t() : color(0) {}
	color_t(int value) : color(value) {}
	color_t(uint8_t r, uint8_t g, uint8_t b, uint8_t a = 0xFF) : color(D3DCOLOR_RGBA(r, g, b, a)) {}
	color_t(Color color) : color(D3DCOLOR_RGBA(color.r(), color.g(), color.b(), color.a())) {}
