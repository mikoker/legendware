...
ImGuiStyle::ImGuiStyle()
{
    ...
    CircleSegmentMaxError   = 1.50f;            // Maximum error (in pixels) allowed when using AddCircle()/AddCircleFilled() or drawing rounded corner rectangles with no explicit segment count specified. Decrease for higher quality but more geometry.
    ...
    ScrollbarOffset = ImVec2(0.0f, 0.0f);
...
static const ImGuiStyleVarInfo GStyleVarInfo[] =
{
    ...
    { ImGuiDataType_Float, 1, (ImU32)IM_OFFSETOF(ImGuiStyle, WindowShadowSize) },    // ImGuiStyleVar_WindowShadowSize
     { ImGuiDataType_Float, 2, (ImU32)IM_OFFSETOF(ImGuiStyle, ScrollbarOffset) },     // ImGuiStyleVar_ScrollbarOffset
};
...
void ImGui::RenderText(ImFont* font, float font_size, ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    // Hide anything after a '##' string
    ...
    if (text != text_display_end)
    {
        window->DrawList->AddText(font, font_size, pos, GetColorU32(ImGuiCol_Text), text, text_display_end);
        if (g.LogEnabled)
            LogRenderedText(&pos, text, text_display_end);
    }
}
void ImGui::RenderTextWrapped(ImFont* font, float font_size, ImVec2 pos, const char* text, const char* text_end, float wrap_width)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (!text_end)
    ...
    if (text != text_end)
    {
        window->DrawList->AddText(font, font_size, pos, GetColorU32(ImGuiCol_Text), text, text_end, wrap_width);
        if (g.LogEnabled)
            LogRenderedText(&pos, text, text_end);
    }
}
...
void ImGui::RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding, ImDrawCornerFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
void ImGui::UpdateMouseMovingWindowNewFrame()
{
    ...
    if (g.MovingWindow != NULL)
    {
        ...
        if (g.IO.MouseDown[0] && IsMousePosValid(&g.IO.MousePos) && (countSubstring(g.MovingWindow->Name, "/" ) == 1 || countSubstring(g.MovingWindow->Name, "/") == 0))
        {
...
ImVec2 ImGui::CalcTextSize(ImFont* font, const char* text, const char* text_end, bool hide_text_after_double_hash, float wrap_width)
{
    ImGuiContext& g = *GImGui;
...
ImVec2 ImGui::CalcTextSize(ImFont* font, float font_size, const char* text, const char* text_end, bool hide_text_after_double_hash, float wrap_width)
{
    ImGuiContext& g = *GImGui;
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            if (flags & ImGuiWindowFlags_DrawShadow)
                window->DrawList->AddShadowRect(window->Pos, window->Pos + window->Size, IM_COL32(0, 0, 0, 100), 30.0f);
        	
            if (flags & ImGuiWindowFlags_RoundLeft)
                window->DrawList->AddRectFilled(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size, bg_col, window_rounding, ImDrawCornerFlags_Left);
            else
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Title bar
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
            RenderWindowTitleBarContents(window, title_bar_rect, window->Name, p_open);
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        ...
        static void NodeDrawCmdShowMeshAndBoundingBox(ImGuiWindow* window, const ImDrawList* draw_list, const ImDrawCmd* draw_cmd, int elem_offset, bool show_mesh, bool show_aabb)
        {
            ...
            ImRect clip_rect = draw_cmd->ClipRect.v;
        ...
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
        {
            ...
            for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
            {
                ...
                ImFormatString(buf, IM_ARRAYSIZE(buf), "DrawCmd:%5d triangles, Tex 0x%p, ClipRect (%4.0f,%4.0f)-(%4.0f,%4.0f)",
                    ...
                    pcmd->ClipRect.v.x, pcmd->ClipRect.v.y, pcmd->ClipRect.v.z, pcmd->ClipRect.v.w);
