...
#define FT_CEIL(X)  (((X + 63) & -64) / 64)
    ...
    bool FreeTypeFont::InitFont(FT_Library ft_library, const ImFontConfig& cfg, unsigned int extra_font_builder_flags)
    {
        ...
        UserFlags = cfg.RasterizerFlags | extra_font_builder_flags;
...
bool    ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags, int* out_dirty_x, int* out_dirty_y, int* out_dirty_width, int* out_dirty_height)
{
    ...
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++)
    {
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++)
        {
            ...
            unsigned int* blit_src = src_glyph.BitmapData;
                unsigned char* blit_dst = (unsigned char*)atlas->TexPixelsAlpha8 + (ty * blit_dst_stride) + tx;
    ...
    for (int buf_i = 0; buf_i < src_tmp_array.Size; buf_i++)
       src_tmp_array[buf_i].~ImFontBuildSrcDataFT();
    src_tmp_array.clear();
...
static void* FreeType_Realloc(FT_Memory /*memory*/, long cur_size, long new_size, void* block)
{
 ...
 bool ImGuiFreeType::ImFontAtlasBuildWithFreeType(ImFontAtlas* atlas, int* out_dirty_x, int* out_dirty_y, int* out_dirty_width, int* out_dirty_height)
{
    ...
    bool ret = ImFontAtlasBuildWithFreeTypeEx(ft_library, atlas, 0U, out_dirty_x, out_dirty_y, out_dirty_width, out_dirty_height);
