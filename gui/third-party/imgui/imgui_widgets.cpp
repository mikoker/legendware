...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    if (text_end - text > 2000 && !wrap_enabled)
    {
        ...
        if (line < text_end)
        {
            ...
            while (line < text_end)
            {
                ...
                text_size.x = ImMax(text_size.x, CalcTextSize(line, line_end).x);
                RenderText(pos - ImVec2(0.0f, 3.0f), line, line_end, false);
    ...
    else
    {
        ...
        RenderTextWrapped(bb.Min - ImVec2(0.0f, 3.0f), text_begin, text_end, wrap_width);
    }
}

void ImGui::TextEx(ImFont* font, float font_size, const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    if (text_end - text > 2000 && !wrap_enabled)
    {
            ...
            if (lines_skippable > 0)
            {
                ...
                while (line < text_end && lines_skipped < lines_skippable)
                {
                    ...
                    if (!line_end)
                        line_end = text_end;
                    if ((flags & ImGuiTextFlags_NoWidthForLargeClippedText) == 0)
                        text_size.x = ImMax(text_size.x, CalcTextSize(font, font_size, line, line_end).x);
                    line = line_end + 1;
                    lines_skipped++;
                }
                pos.y += lines_skipped * line_height;
        ...
        if (line < text_end)
        {
            ...
            while (line < text_end)
            {
                ...
                text_size.x = ImMax(text_size.x, CalcTextSize(font, font_size, line, line_end).x);
                RenderText(font, font_size, pos - ImVec2(0.0f, 3.0f), line, line_end, false);
    ...
    {
        const float wrap_width = wrap_enabled ? CalcWrapWidthForPos(window->DC.CursorPos, wrap_pos_x) : 0.0f;
        const ImVec2 text_size = CalcTextSize(font, font_size, text_begin, text_end, false, wrap_width);
        ...
        // Render (we don't hide text after ## in this end-user function)
        RenderTextWrapped(font, font_size, bb.Min - ImVec2(0.0f, 3.0f), text_begin, text_end, wrap_width);
    }
}

void ImGui::Text(ImFont* font, float font_size, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    TextV(font, font_size, fmt, args);
    va_end(args);
}


void ImGui::TextV(ImFont* font, float font_size, const char* fmt, va_list args)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    TextEx(font, font_size, g.TempBuffer, text_end, ImGuiTextFlags_NoWidthForLargeClippedText);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    bb.Min += g.Style.ScrollbarOffset;
    bb.Max += ImVec2(g.Style.ScrollbarOffset.x, 0.0f);
	...
	window->DrawList->AddRectFilled(bb.Min - ImVec2(1.0f, 1.0f), bb.Max + ImVec2(1.0f, 1.0f),
        GetColorU32(ImGuiCol_PlotLines), window->WindowRounding, ImDrawCornerFlags_All);

	window->DrawList->AddRectFilled(bb.Min, bb.Max, bg_col, window->WindowRounding, ImDrawCornerFlags_All);
	...
	window->DrawList->AddRectFilled(grab_rect.Min - ImVec2(1.0f, 1.0f), 
        grab_rect.Max + ImVec2(1.0f, 1.0f), GetColorU32(ImGuiCol_PlotLines), style.ScrollbarRounding);
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col, float rounding)
{
    if (!user_texture_id)
        return;
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = GetFrameHeight() - 10.0f;
    ...
    window->DrawList->AddRect(check_bb.Min, check_bb.Max,
        GetColorU32(ImGuiCol_FrameBgActive), 1.0f, ImDrawCornerFlags_All, 2.5f);
	
    RenderFrame(check_bb.Min + ImVec2(1.0f, 1.0f), check_bb.Max - ImVec2(1.0f, 1.0f),
        GetColorU32(*v ? ImGuiCol_FrameBgActive : ImGuiCol_FrameBg), false, 0.0f);
	...
	if (label_size.x > 0.0f)
	{
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 7.0f,
            check_bb.Min.y + style.FramePadding.y - 5.0f), label);
	}
...
namespace u
{
	void fade(bool toggle, float& result,
        float min, float max, float speed = 5.5f);
...
namespace core::f { ImFont* awesome = nullptr; }
namespace core::s { int temp_dpi_scale = 2; }


bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, float* expected_h, bool* close)
{
    ...
    const ImVec2 label_size = CalcTextSize(g.Font,
        u::dpi(g.FontSize), label, nullptr, true);

    const float expected_w = CalcItemWidth();
    const float w = ((flags & ImGuiComboFlags_NoPreview) ? arrow_size : expected_w) + u::dpi(1.0f);

    const bool has_shit = strstr(label, "##") != nullptr;

    const ImRect frame_bb(window->DC.CursorPos - u::dpi(ImVec2(1.0f, has_shit ? 15.0f : 1.0f)),
        window->DC.CursorPos + ImVec2(w - u::dpi(1.0f), u::dpi(40.0f - (has_shit ? 15.0f : 0.0f))));

    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ?
        style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f) - u::dpi(ImVec2(0.0f, has_shit ? 5.0f : 0.0f)));
    ...
    ImRect inter_bb = frame_bb;
    inter_bb.Min += ImVec2(0.0f, u::dpi(13.0f));

    bool hovered, held;
    bool pressed = ButtonBehavior(inter_bb, id, &hovered, &held);

    bool popup_open = IsPopupOpen(id, ImGuiPopupFlags_None);

    RenderNavHighlight(inter_bb, id);

    if (!(flags & ImGuiComboFlags_NoPreview))
        RenderFrame(inter_bb.Min, inter_bb.Max, GetColorU32(ImGuiCol_Button), false, 3.0f);

    const ImVec2 preview_size = CalcTextSize(g.Font,
        u::dpi(g.FontSize), preview_value, nullptr, true);

    if (preview_value != nullptr && !(flags & ImGuiComboFlags_NoPreview))
    {
        window->DrawList->AddText(g.Font, u::dpi(g.FontSize), inter_bb.GetCenter() - ImVec2(u::dpi(90.0f),
            preview_size.y / 2.0f), GetColorU32(ImGuiCol_Text), preview_value);
    }

    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        window->DrawList->AddText(core::f::awesome, u::dpi(6.2f), inter_bb.GetCenter() - ImVec2(u::dpi(-80.0f),
            preview_size.y / 4.0f - u::dpi(5.0f)), GetColorU32(ImGuiCol_ComboArrow), (char*)u8"\uf078");
    }

    if (label_size.x > 0 && !has_shit)
    {
        window->DrawList->AddText(g.Font, u::dpi(g.FontSize), ImVec2(frame_bb.Min.x - u::dpi(1.0f),
            frame_bb.Min.y - u::dpi(10.0f)), GetColorU32(ImGuiCol_Text), label);
    }
    ...
    if (flags & ImGuiComboFlags_ColorPicker && g.DragDropActive)
    {
        if (close != nullptr)
        {
            ImRect bb = inter_bb;

            // window->DrawList->AddRect(bb.Min, bb.Max, IM_COL32A(255, 0, 0));

            *close = !IsMouseHoveringRect(bb.Min, bb.Max);
        }
    ...
    else
    {
        ...
        popup_max_height_in_items = 6;
        SetNextWindowSizeConstraints(ImVec2(w, 0.0f), ImVec2(FLT_MAX, u::dpi(CalcMaxPopupHeightFromItemCount(popup_max_height_in_items))));
    }
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
    {
        if (popup_window->WasActive)
        {
            ...
            exp_h = size_expected.y;

            if (expected_h != nullptr)
                *expected_h = exp_h;

            u::fade(false, value, 1.0f, size_expected.y);

            ImRect r_outer = GetWindowAllowedExtentRect(popup_window);
            ImVec2 pos = FindBestWindowPosForPopupEx(frame_bb.GetBL(), size_expected,
                &popup_window->AutoPosLastDirection, r_outer, frame_bb, ImGuiPopupPositionPolicy_ComboBox);

            SetNextWindowPos(pos);
            SetNextWindowSize(ImVec2(0.0f, value));
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags, const ImVec2& size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (p_step != NULL)
    {
        ...
        if (InputText("", buf, IM_ARRAYSIZE(buf), flags, nullptr, nullptr, size)) // PushId(label) + "" gives us the expected ID from outside point of view
...
bool ImGui::InputScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags, const ImVec2& size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        if (i > 0)
            SameLine(0, g.Style.ItemInnerSpacing.x);
        value_changed |= InputScalar("", data_type, p_data, p_step, p_step_fast, format, flags, size);
        PopID();
        PopItemWidth();
        p_data = (void*)((char*)p_data + type_size);
    }
...
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast, const char* format, ImGuiInputTextFlags flags, const ImVec2& size)
{
    flags |= ImGuiInputTextFlags_CharsScientific;
    return InputScalar(label, ImGuiDataType_Float, (void*)v, (void*)(step > 0.0f ? &step : NULL), (void*)(step_fast > 0.0f ? &step_fast : NULL), format, flags, size);
}

bool ImGui::InputFloat2(const char* label, float v[2], const char* format, ImGuiInputTextFlags flags, const ImVec2& size)
{
    return InputScalarN(label, ImGuiDataType_Float, v, 2, NULL, NULL, format, flags, size);
}

bool ImGui::InputFloat3(const char* label, float v[3], const char* format, ImGuiInputTextFlags flags, const ImVec2& size)
{
    return InputScalarN(label, ImGuiDataType_Float, v, 3, NULL, NULL, format, flags, size);
}

bool ImGui::InputFloat4(const char* label, float v[4], const char* format, ImGuiInputTextFlags flags, const ImVec2& size)
{
    return InputScalarN(label, ImGuiDataType_Float, v, 4, NULL, NULL, format, flags, size);
}

// Prefer using "const char* format" directly, which is more flexible and consistent with other API.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast, int decimal_precision, ImGuiInputTextFlags flags, const ImVec2& size)
{
    char format[16] = "%f";
    if (decimal_precision >= 0)
        ImFormatString(format, IM_ARRAYSIZE(format), "%%.%df", decimal_precision);
    return InputFloat(label, v, step, step_fast, format, flags, size);
}

bool ImGui::InputFloat2(const char* label, float v[2], int decimal_precision, ImGuiInputTextFlags flags, const ImVec2& size)
{
    char format[16] = "%f";
    if (decimal_precision >= 0)
        ImFormatString(format, IM_ARRAYSIZE(format), "%%.%df", decimal_precision);
...
bool ImGui::InputFloat3(const char* label, float v[3], int decimal_precision, ImGuiInputTextFlags flags, const ImVec2& size)
{
    char format[16] = "%f";
    if (decimal_precision >= 0)
        ImFormatString(format, IM_ARRAYSIZE(format), "%%.%df", decimal_precision);
...
bool ImGui::InputFloat4(const char* label, float v[4], int decimal_precision, ImGuiInputTextFlags flags, const ImVec2& size)
{
    char format[16] = "%f";
    if (decimal_precision >= 0)
        ImFormatString(format, IM_ARRAYSIZE(format), "%%.%df", decimal_precision);
...
bool ImGui::InputInt(const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags flags, const ImVec2& size)
{
    // Hexadecimal input provided as a convenience but the flag name is awkward. Typically you'd use InputText() to parse your own data, if you want to handle prefixes.
    const char* format = (flags & ImGuiInputTextFlags_CharsHexadecimal) ? "%08X" : "%d";
    return InputScalar(label, ImGuiDataType_S32, (void*)v, (void*)(step > 0 ? &step : NULL), (void*)(step_fast > 0 ? &step_fast : NULL), format, flags, size);
}

bool ImGui::InputInt2(const char* label, int v[2], ImGuiInputTextFlags flags, const ImVec2& size)
{
    return InputScalarN(label, ImGuiDataType_S32, v, 2, NULL, NULL, "%d", flags, size);
}

bool ImGui::InputInt3(const char* label, int v[3], ImGuiInputTextFlags flags, const ImVec2& size)
{
    return InputScalarN(label, ImGuiDataType_S32, v, 3, NULL, NULL, "%d", flags, size);
}

bool ImGui::InputInt4(const char* label, int v[4], ImGuiInputTextFlags flags, const ImVec2& size)
{
    return InputScalarN(label, ImGuiDataType_S32, v, 4, NULL, NULL, "%d", flags, size);
}

bool ImGui::InputDouble(const char* label, double* v, double step, double step_fast, const char* format, ImGuiInputTextFlags flags, const ImVec2& size)
{
    flags |= ImGuiInputTextFlags_CharsScientific;
    return InputScalar(label, ImGuiDataType_Double, (void*)v, (void*)(step > 0.0 ? &step : NULL), (void*)(step_fast > 0.0 ? &step_fast : NULL), format, flags, size);
}
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, const ImVec2& size)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx(label, NULL, buf, (int)buf_size, size, flags, callback, user_data);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(g.Font, u::dpi(g.FontSize), label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(u::dpi(size_arg), CalcItemWidth(), (is_multiline ? font_size * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    if (g.ActiveId == id)
    {
        ...
        const float mouse_y = (is_multiline ? (io.MousePos.y - draw_window->DC.CursorPos.y - style.FramePadding.y) : (font_size * 0.5f));
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const int row_count_per_page = ImMax((int)((inner_size.y - style.FramePadding.y) / font_size), 1);
        ...
        else if (IsKeyPressedMap(ImGuiKey_UpArrow) && is_multiline)     { if (io.KeyCtrl) SetScrollY(draw_window, ImMax(draw_window->Scroll.y - font_size, 0.0f)); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTSTART : STB_TEXTEDIT_K_UP) | k_mask); }
        else if (IsKeyPressedMap(ImGuiKey_DownArrow) && is_multiline)   { if (io.KeyCtrl) SetScrollY(draw_window, ImMin(draw_window->Scroll.y + font_size, GetScrollMaxY())); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTEND : STB_TEXTEDIT_K_DOWN) | k_mask); }
        else if (IsKeyPressedMap(ImGuiKey_PageUp) && is_multiline)      { state->OnKeyPressed(STB_TEXTEDIT_K_PGUP | k_mask); scroll_y -= row_count_per_page * font_size; }
        else if (IsKeyPressedMap(ImGuiKey_PageDown) && is_multiline)    { state->OnKeyPressed(STB_TEXTEDIT_K_PGDOWN | k_mask); scroll_y += row_count_per_page * font_size; }
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            cursor_offset.y = searches_result_line_no[0] * font_size;
            if (searches_result_line_no[1] >= 0)
            {
                select_start_offset.x = InputTextCalcTextSizeW(ImStrbolW(searches_input_ptr[1], text_begin), searches_input_ptr[1]).x;
                select_start_offset.y = searches_result_line_no[1] * font_size;
            }

            // Store text height (note that we haven't calculated text width at all, see GitHub issues #383, #1224)
            if (is_multiline)
                text_size = ImVec2(inner_size.x, line_count * font_size);
        }
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (is_multiline)
            {
                ...
                if (cursor_offset.y - font_size < scroll_y)
                    scroll_y = ImMax(0.0f, cursor_offset.y - font_size);
        ...
        if (render_selection)
        {
            ...
            {
                if (rect_pos.y > clip_rect.w + font_size)
                    break;
                ...
                else
                {
                    ...
                    ImRect rect(rect_pos + ImVec2(0.0f, bg_offy_up - font_size), rect_pos + ImVec2(rect_size.x, bg_offy_dn));
                    rect.ClipWith(clip_rect);
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, bg_color);
                }
                rect_pos.x = draw_pos.x - draw_scroll.x;
                rect_pos.y += font_size;
            }
        }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(g.Font, font_size, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - font_size + 0.5f, cursor_screen_pos.x + 1.0f, cursor_screen_pos.y - 1.5f);
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), GetColorU32(ImGuiCol_Text));

            // Notify OS of text input position for advanced IME (-1 x offset so that Windows IME can cover our cursor. Bit of an extra nicety.)
            if (!is_readonly)
                g.PlatformImePos = ImVec2(cursor_screen_pos.x - 1.0f, cursor_screen_pos.y - font_size);
        }
    }
    else
    {
        ...
        if (is_multiline)
            text_size = ImVec2(inner_size.x, InputTextCalcTextLenAndLineCount(buf_display, &buf_display_end) * font_size); // We don't need width
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(g.Font, font_size, draw_pos, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
    ...
    if (label_size.x > 0)
    {
        window->DrawList->AddText(g.Font, u::dpi(g.FontSize), ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x,
            frame_bb.Min.y + style.FramePadding.y), GetColorU32(ImGuiCol_Text), label);
...
{
    const bool result = ColorEdit4(label, col, flags | ImGuiColorEditFlags_NoAlpha);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                OpenPopup("picker");
                SetNextWindowPos(window->DC.LastItemRect.GetTR() + ImVec2(15.0f, 0.0f));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick("context");
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
    ...
    ImVec2 size = CalcItemSize(u::dpi(core::s::temp_dpi_scale < 2 ? size_arg + ImVec2(0.0f, 10.0f) : size_arg),
        CalcItemWidth(), GetTextLineHeightWithSpacing() * 7.4f + style.ItemSpacing.y);
...
void ImGui::SetWindowClipRectBeforeSetChannel(ImGuiWindow* window, const ImRect& clip_rect)
{
    ...
    window->DrawList->_CmdHeader.ClipRect.v = clip_rect_vec4;
    window->DrawList->_CmdHeader.ClipRect.ClipRectCircle = false;
    window->DrawList->_ClipRectStack.Data[window->DrawList->_ClipRectStack.Size - 1] = clip_rect_vec4;
    window->DrawList->_ClipRectStackCircle.Data[window->DrawList->_ClipRectStackCircle.Size - 1] = false;
}
