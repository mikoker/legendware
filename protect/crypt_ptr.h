...
public:
	...
	{
		key = RND;
		xor_value = crc32_table[*((uint8_t*)&ptr + 1) + ((((uintptr_t)ptr >> 16) & 7) << 8)];
		np = xor_value ^ ((uintptr_t)ptr + crc32_table[key % 256]);
	}

	__forceinline T* get()
	{
		return (T*)((uintptr_t)(np ^ xor_value) - crc32_table[key % 256]);
	}
	...
	{
		return (uintptr_t)(np ^ xor_value) - crc32_table[key % 256];
	}
};
#else
template <typename T>
