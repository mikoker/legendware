...
public:
	...
	virtual void*                 SaveAllocMemory(size_t num, size_t size) = 0; // 76
	virtual void                  SaveFreeMemory(void *pSaveMem) = 0;//77
...
public:
	...
	virtual bool		IsLoopback(void) const = 0;	// true if loopback channel 6
	virtual bool		IsTimingOut(void) const = 0;	// true if timing out 7
	virtual bool		IsPlayback(void) const = 0;	// true if demo playback 8

	virtual float		GetLatency(int flow) const = 0;	 // current latency (RTT), more accurate but jittering 9
	virtual float		GetAvgLatency(int flow) const = 0; // average packet latency in seconds 10
	virtual float		GetAvgLoss(int flow) const = 0;	 // avg packet loss[0..1] 11
	virtual float		GetAvgChoke(int flow) const = 0;	 // avg packet choke[0..1] 12
	virtual float		GetAvgData(int flow) const = 0;	 // data flow in bytes/sec 13
	virtual float		GetAvgPackets(int flow) const = 0; // avg packets/sec 14
	virtual int			GetTotalData(int flow) const = 0;	 // total flow in/out in bytes 15
	virtual int			GetTotalPackets(int flow) const = 0; // 16
	virtual int			GetSequenceNr(int flow) const = 0;	// last send seq number 17
	virtual bool		IsValidPacket(int flow, int frame_number) const = 0; // true if packet was not lost/dropped/chocked/flushed 18
	virtual float		GetPacketTime(int flow, int frame_number) const = 0; // time when packet was send 19
	virtual int			GetPacketBytes(int flow, int frame_number, int group) const = 0; // group size of this packet 20
	virtual bool		GetStreamProgress(int flow, int *received, int *total) const = 0;  // TCP progress if transmitting 21
	...
	bool SendNetMsg(NetworkMessage* msg, bool bForceReliable, bool bVoice = false) {
		using SendNetMsg_t = bool* (__thiscall*)(void*, NetworkMessage*, bool, bool);
		return call_virtual<SendNetMsg_t>(this, 40)(this, msg, bForceReliable, bVoice);
	}
