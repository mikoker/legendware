...
public:
    bool ShouldHitEntity(IHandleEntity * pEntityHandle, int /*contentsMask*/)
    {
		...
		{
			if (!strcmp(eCC->m_pNetworkName, ccIgnore))
				return false;
		}
...
public:
    virtual bool ShouldHitEntity(IHandleEntity* pEntityHandle, int contentsMask) {
        ...
        if (clclass) {
            if (crypt_hash_r(clclass->m_pNetworkName) == crypt_hash("CBaseCSGrenadeProjectile"))
                return false;
...
public:
    ...
    {
        return !((IClientEntity*)hit_entity)->EntIndex();
    }

    bool DidHitNonWorldEntity() const
    {
        return (IClientEntity*)hit_entity && !DidHitWorld();
    }
public:
