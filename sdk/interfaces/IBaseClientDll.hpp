...
public:
	...
	virtual void AddPlayerToVoiceList(uint64_t xPlayer, int iController, uint64_t uiFlags) = 0;
	virtual void RemovePlayerFromVoiceList(uint64_t xPlayer, int iController) = 0;

	virtual void GetRemoteTalkers(int* pNumTalkers, uint64_t* pRemoteTalkers) = 0;

	virtual bool VoiceUpdateData(int iController) = 0;
	virtual void GetVoiceData(int iController, const unsigned char** ppvVoiceDataBuffer, unsigned int* pnumVoiceDataBytes) = 0;
	virtual void VoiceResetLocalData(int iController) = 0;

	virtual void SetPlaybackPriority(uint64_t remoteTalker, int iController, int iAllowPlayback) = 0;
	virtual void PlayIncomingVoiceData(uint64_t xuid, const unsigned char* pbData, unsigned int dwDataSize, const bool* bAudiblePlayers = NULL) = 0;
