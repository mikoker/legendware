...
void RayTracer::TraceHitbox(const Ray& ray, const Hitbox& hitbox, Trace& trace, int flags) {
	...
	if (InputSys::Get()->IsKeyDown(VirtualKeys::F)) {
		Source::m_pDebugOverlay->AddBoxOverlay(minsStart, Vector(-1.0f, -1.0f, -1.0f), Vector(1.0f, 1.0f, 1.0f), QAngle(), 255, 255, 0, 255, 1.0f);
		Source::m_pDebugOverlay->AddBoxOverlay(maxsStart, Vector(-1.0f, -1.0f, -1.0f), Vector(1.0f, 1.0f, 1.0f), QAngle(), 255, 0, 255, 255, 1.0f);
	}
#endif
