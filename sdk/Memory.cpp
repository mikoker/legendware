...
{
	return reinterpret_cast<T>(reinterpret_cast<char*>(address + 1) + *address); //-V206
}
...
void Memory::initialize() noexcept
{
	...
	hud = *temp;
	findHudElement_ = relativeToAbsolute<decltype(findHudElement_)>(reinterpret_cast<int*>(reinterpret_cast<char*>(temp) + 5)); //-V206
	clearHudWeapon = FIND_PATTERN(decltype(clearHudWeapon), crypt_str("client"), crypt_str("\x55\x8B\xEC\x51\x53\x56\x8B\x75\x08\x8B\xD9\x57\x6B\xFE\x34"));
